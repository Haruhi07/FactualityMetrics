...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        fact_score = torch.sum(preds_prob, dim=0)[0]/len(preds_prob)
        ...
        with open('tmp/probs.txt','w') as f:
            f.write(str(preds_prob.detach().cpu().numpy().tolist()))
...
def main():
    ...
    parser.add_argument("--fact_score_dir", default=None, type=str, required=True,
                        help="The directory for saving fact_score")
